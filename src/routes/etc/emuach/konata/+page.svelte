<script>
  ...
  import { emuach_konata_num } from "$lib/store/store.js"
  import { fly } from '../components/fly'
  import konata from "./konata.jpg"
  import emuach_img from "./emuach.jpg"
  ...
  function increment_emuach(){
    emuach_num_int += 1
    emuaches += 1
    $emuach_konata_num = emuach_num_int
  ...
  $: {
    emuach_num_int = parseInt($emuach_konata_num)
  }
...
<div class="flex-middle w-screen h-screen flex-col gap-5 ">
  ...
  <h1 class="text-white flex-middle flex-row h-auto w-auto z-50" >{$emuach_konata_num}
    <span>
  ...
  <div class="flex-middle h-[300px] w-[300px]">
    <img src={konata} width="200px" alt="">
    {#each emuach_arr as em}
      {#if emuaches === em && emuached}
